...
- Configurable rate limiting and timeout controls for API requests.
...
_If you install only the dependencies, run the program using_ `python -m travecto.cli` _(or_ `python -OO travecto/cli.py`_) instead of the_ `travecto` _command._
...
Modify `config.py` to add your own cities and places of interest.
